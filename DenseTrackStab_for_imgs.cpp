...
int main(int argc, char** argv)
{
	...
	string videoname_prefix("/home/lijun/tvcj/kth_frames/TEST");
	string save_videoname_prefix("/home/lijun/tvcj/kth_features_all/");

	get_folders(videoname_prefix,videonames);
	
	for(int myclass = 0 ; myclass<1; myclass++)
{
//////////////// 每隔X个读视频////
...
///////////////////////////	
	...
	for (int num = 0;num<videonames.size();num++) //循环所有视频
	{
		gettimeofday(&onevideo_starttime, NULL);
		string video = videonames[num];
		string name = videonames[num];
		std::cout << "video name is:  "<<name<<std::endl;
		name.erase(name.begin(),name.begin() + name.rfind("/") + 1);
		name = save_videoname_prefix + name + ".txt";
...
//	fprintf(stderr, "video size, length: %d, width: %d, height: %d\n", seqInfo.length, seqInfo.width, seqInfo.height);
	...
	std::vector<string> img_frames;
	get_files(video, img_frames);
	
	int video_length = img_frames.size();
	for(int img_num=0; img_num < img_frames.size(); img_num++) {
		Mat frame;
		int i, j, c;

		// get a new frame
		frame = imread(img_frames[img_num], CV_LOAD_IMAGE_COLOR);
		if(frame.empty())
			break;
		...
		for(int iScale = 0; iScale < scale_num; iScale++) {
			...
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				//		printf("%d\t%f\t%f\t%f\t%f\t%f\t%f\t", frame_num, mean_x, mean_y, var_x, var_y, length, fscales[iScale]);
						...
						file<< std::min<float>(std::max<float>(mean_x/float(frame.cols), 0), 0.999)<<"\t"<<std::min<float>(std::max<float>(mean_y/float(frame.rows), 0), 0.999)<<"\t"<<std::min<float>(std::max<float>((frame_num - trackInfo.length/2.0 - start_frame)/float(video_length), 0), 0.999)<<"\t";
