...
void GetRect(const Point2f& point, RectInfo& rect, const int width, const int height, const DescInfo& descInfo)
{
	...
	//point.x or point.y <x_min y_min , rect.x and rect.y =0; point.x or point.y>x_max y_max , rect.x and rect.y =0;
...
void GetDesc(const DescMat* descMat, RectInfo& rect, DescInfo descInfo, std::vector<float>& desc, const int index)
{
	...
	for(int yPos = rect.y, y = 0; y < descInfo.nyCells; yPos += yStride, y++) {
		...
		for(int i = 0; i < nBins; i++) {
			...
			vec[iDesc++] = std::max<float>(sum, 0) + epsilon;  //归一化时需加上的小误差e
		}
	}
...
bool IsValid(std::vector<Point2f>& track, float& mean_x, float& mean_y, float& var_x, float& var_y, float& length)
{
	...
	if(cur_max > max_dis && cur_max > length*0.7)   //消除运动范围大的轨迹
		return false;

	track.pop_back();   //track 有16个数  删除最后一个数据  得到15*2维向量
	norm = 1./length;
...
{
	int width = grey.cols/min_distance;  //min_dis = 5 
	int height = grey.rows/min_distance;

	Mat eig;
	cornerMinEigenVal(grey, eig, 3, 3); //calculates the minimal eigenvalue of gradient matrices for corner detection
	...
	int x_max = min_distance*width;  //cols
	int y_max = min_distance*height; //rows
...
void InitPry(const Mat& frame, std::vector<float>& scales, std::vector<Size>& sizes)
{
	...
	int nlayers = 0;
	while(min_size >= patch_size) {    //32------ patch_size    decide howw many nlayers
		min_size /= scale_stride;     //sqrt(2)
		nlayers++;
	}
...
void PrintDesc(std::vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo, std::ofstream &fs)
{
...
void LoadBoundBox(string file, std::vector<Frame>& bb_list)
{
	// load the bouding box file
    std::ifstream bbFile(file.c_str());
    std::string line;
	            			 ...
	            			 int borderType = BORDER_CONSTANT, const Scalar& borderValue = Scalar())
{
    ...
    int bw0 = std::min(BLOCK_SZ*BLOCK_SZ/bh0, width);   //64 width
    bh0 = std::min(BLOCK_SZ*BLOCK_SZ/bw0, height);         //16 height
				  ...
				  const Mat& prev_desc, const Mat& desc, std::vector<Point2f>& prev_pts, std::vector<Point2f>& pts)
{
	...
	Mat mask = windowedMatchingMask(kpts, prev_kpts, 25, 25);   //前一帧后一帧对应点的变化不超过25个像素
...
void get_folders(string path,vector<string>& files)
{
    string p;
    struct stat statbuf;
    // check the parameter !
